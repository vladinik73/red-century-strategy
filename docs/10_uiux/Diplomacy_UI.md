...
- Предложение помещается в очередь.
- Показывается снова при открытии панели Дипломатия в начале следующего хода игрока, или как отдельный reminder.
- Поведение: не блокирует ход AI; игрок должен ответить до конца своего следующего хода (или предложение считается отклонённым).
